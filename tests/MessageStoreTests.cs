        ...
        public void TestMessageStore()
        {
            ...
            for (int run = 1; run <= 3; ++run)
            {
                using (var db = new MessageStore(db_file_path))
                {
                    ...
                    Assert.IsNull(db.GetMessage("bad@bad.com", "pwd", out recv_payload_file_path, out recv_payload_file_hash));

                    msg? recv_msg = db.GetMessage(send_msg.to, "pwd", out recv_payload_file_path, out recv_payload_file_hash);
                    if (recv_msg == null) 
